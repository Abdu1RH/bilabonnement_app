...
function AvailableCar() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/cars') // Replace with the actual API endpoint for cars
      .then(response => {
  ...
  const totalPriceRentedCars = totalDetailsRentedCars.reduce((total, car) => total + car.price, 0);
      ...
      <div className="outer-container">
        ...
        <div className="car-container">
          <ul className="rented-car-list">
            ...
            {totalDetailsRentedCars.map(car => (
              <p key={car.id}>{car.id} - {car.brand} - {car.price} kr.</p>
            ))}
